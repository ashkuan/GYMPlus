            ...
            <div class="col-12 col-lg-6">
              <div class="card-body d-flex flex-column justify-content-between h-100">
                <ul>
                  ...
                  <li>
                    <p class="card-text">授課教練：{{ product?.coach }}</p>
                  </li>
                  <li>
                    <p class="card-text">難易度：{{ product?.difficulty }}</p>
                  </li>
                  <li>
                    <p class="card-text">適合對象：{{ joinedString }}</p>
                  </li>
                ...
                <div class="d-flex flex-column">
                  ...
                  <button
                    type="button"
                    class="btn btn-outline-danger rounded align-self-end w-50"
                    v-if="this.carts.some((id) => id.product_id === this.id)"
                    disabled
                  >
                    已加入購物車
                  </button>
...
export default {
  ...
  methods: {
    getData() {
      let data = [];
      this.axios.get(`${this.url}api/${this.path}/products/all`).then((res) => {
